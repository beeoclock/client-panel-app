		...
		switchMap((item) => combineLatest([
			this.store.select(BusinessProfileState.earliestScheduleAndLatestSchedule).pipe(
				...
				tap(({earliestSchedule, latestSchedule}) => {
					...
					this.weekCalendarLocaStateService.setEarliestScheduleInSeconds(earliestSchedule.startInSeconds);
					this.weekCalendarLocaStateService.setLatestScheduleInSeconds(latestSchedule.endInSeconds);
			...
			this.store.select(BusinessProfileState.schedules).pipe(
				...
				tap((schedules) => {
					this.weekCalendarLocaStateService.setSchedules(schedules);
				})
			)
		]).pipe(map(() => item)))
	);
