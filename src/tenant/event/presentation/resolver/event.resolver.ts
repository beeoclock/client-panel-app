...
export const eventResolver: ResolveFn<IEvent_V2<{
	...
	return from(sharedUow.order.findByServiceIds([orderedServiceId])).pipe(
		...
		map(([order]) => {
			const service = order.services.find(({_id}) => _id === orderedServiceId) as IOrderServiceDto;
		...
		filter(is.not_null_or_undefined<IEvent_V2<{ order: IOrder.DTO; service: IOrderServiceDto; }>>),
	);
