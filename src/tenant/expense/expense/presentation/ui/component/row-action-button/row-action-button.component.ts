...
@Component({
	selector: 'expense-row-action-button-component',
	standalone: true,
...
export class RowActionButtonComponent {
	...
	public readonly item = input.required<IExpense.EntityRaw>();
	...
	public setState(state: StateEnum) {
		return new ExpenseDataActions.SetState(this.item(), state);
	}

	public open(): void {
		this.store.dispatch(new ExpensePresentationActions.OpenDetails(this.item()));
	}

	public edit(): void {
		this.store.dispatch(new ExpensePresentationActions.OpenForm({
			componentInputs: {
				isEditMode: true,
				item: this.item()
			}
