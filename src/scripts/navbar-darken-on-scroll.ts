...
export const navbarDarkenOnScroll = () => {
  const Selector = {
  ...
  const toggleThemeClass = (theme: 'dark' | 'light') => {
    if (theme === 'dark') {
  ...
  if (navbar) {
    ...
    const navbarCollapse: HTMLElement = navbar.querySelector(Selector.NAVBAR_COLLAPSE) as any;
    const colorRgb: any = utils.hexToRgb(
    ...
    utils.resize(() => {
      ...
      if (window.innerWidth > breakPoint) {
        ...
        utils.hasClass(
          navbar.querySelector(Selector.NAVBAR_TOGGLER) as any,
