export const searchInit = () => {
  ...
  const hideSearchSuggestion = (searchArea: any) => {
  ...
  searchAreas.forEach((searchArea) => {
    const input: HTMLInputElement = searchArea.querySelector(Selectors.SEARCH_INPUT) as any;
    ...
    document.addEventListener(Events.CLICK, ({target}) => {
      !searchArea.contains(target as Node) && hideSearchSuggestion(searchArea);
    });

    btnDropdownClose &&
    btnDropdownClose.addEventListener(Events.CLICK, (e: any) => {
