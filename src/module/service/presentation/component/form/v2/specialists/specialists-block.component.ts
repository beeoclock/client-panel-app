			...
			<div *ngFor="let member of specialistListControl.value; let index = index">
				...
				<div class="grid grid-cols-16 gap-3">
					...
					<div class="col-span-4 flex items-start justify-end">
						<button type="button" (click)="removeMemberFromSelectedList(member?.member)"
...
export class SpecialistsBlockComponent implements OnInit {
	...
	@Input()
	public specialistListControl: FormControl<ISpecialist[]> = new FormControl([] as any);
	...
	private async initSpecialists() {
		...
		if (!this.specialistListControl.value.length) {
			...
			if (this.modalSelectSpecialistListAdapter.tableState.total === 1) {
				...
				this.specialistListControl.patchValue([Service.memberToSpecialist(this.modalSelectSpecialistListAdapter.tableState.items[0])]);
	...
	public removeMemberFromSelectedList(member: RIMember | null | undefined): void {
