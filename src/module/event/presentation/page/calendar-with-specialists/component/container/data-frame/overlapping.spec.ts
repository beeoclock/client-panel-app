...
import {RIEvent} from "@event/domain"; // Припустимо, що ваші функції знаходяться в цьому файлі
...
describe('Event Utils', () => {
	describe('isOverlapping', () => {
		it('should return true when events overlap', () => {
			...
			expect(isOverlapping(event1 as unknown as RIEvent, event2 as unknown as RIEvent)).toBe(true);
		});
		...
		it('should return false when events do not overlap', () => {
			...
			expect(isOverlapping(event1 as unknown as RIEvent, event2 as unknown as RIEvent)).toBe(false);
		});
	});
	...
	describe('groupOverlappingEvents', () => {
		it('should group overlapping events together', () => {
			...
			const groupedEvents = groupOverlappingEvents(events as unknown as RIEvent[]);
