...
import {ListCustomerApi} from "@customer/index";
...
export class EventListCustomerRepository {
	...
	public readonly tableState = TableState.create<ICustomer.DTO>({
		filters: {
	...
	public async getPageAsync() {
		...
		try {
			...
			this.tableState
				.nextPage()
				.setItems(([] as ICustomer.DTO[]).concat(this.tableState.items, data.items))
